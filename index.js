...
const { buildArgv } = require("jest/node_modules/jest-cli/build/cli");
...
// It can access 'internal' because both the variable and 'nestedFunction' are nested in the function 'myFunction'
...
function lowerCaseNames(array) {
  const nameLowerCase = array.map((item) => item.animal_name.toLowerCase());
...
function lowPopulationAnimals(array) {
  const animalsLowPop = array.filter((item) => item.population < 5);
...
function USApop(array) {
  const popUSA = array.reduce((acc, item) => {
